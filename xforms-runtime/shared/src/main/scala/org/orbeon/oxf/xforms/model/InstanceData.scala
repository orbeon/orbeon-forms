...
object InstanceData {
  ...
  private val ReadonlyLocalInstanceData: InstanceData = new InstanceData {
    override def getRequired: Boolean = Types.DEFAULT_REQUIRED
    override def getValid: Boolean = Types.DEFAULT_VALID
...
class InstanceData private {
  ...
  private def getLocalRelevant: Boolean =
    testMipForNonDefault(_.relevant, Types.DEFAULT_RELEVANT)

  private def getLocalReadonly: Boolean =
    testMipForNonDefault(_.readonly, Types.DEFAULT_READONLY)

  def getRequired: Boolean =
    testMipForNonDefault(_.required, Types.DEFAULT_REQUIRED)

  def getValid: Boolean =
    ! schemaInvalid && testMipForNonDefault(_.valid, Types.DEFAULT_VALID)
  ...
  def getInvalidBindIds: List[String] = {
    ...
    if ((bindNodes ne null) && ! bindNodes.isEmpty) {
      ...
      while (it.hasNext) {
        ...
        if (bindNode.valid != Types.DEFAULT_VALID) {
          if (sb eq null)
