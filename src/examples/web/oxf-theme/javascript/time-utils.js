...
function checkHour(hobj) {
  ...
  if (isNaN(hobj.value)) {
     ...
     if (hobj.value.length == 2)    {
       if (!upRange(hobj,23))   {
         alert("There is no Hour Greater than 23");
         hobj.value = 23;
         hobj.focus();
        }
...
function checkMinute(mobj) {
  ...
  if (isNaN(mobj.value))
    ...
    if (mobj.value.length == 2)   {
       if (!upRange(mobj,59))  {
         alert("There is no Minute Greater than 59");
         mobj.value = 59;
         mobj.focus();
        }
