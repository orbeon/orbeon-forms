...
abstract class SimpleAnalysis(val staticState: XFormsStaticState, val scope: XBLBindings#Scope, val element: Element,
    ...
    val prefixedId = if (element != null) scope.getPrefixedIdForStaticId(XFormsUtils.getElementStaticId(element)) else "#controls"
    ...
    protected def computeBindingAnalysis(element: Element): XPathAnalysis = {
        if (element != null) {
            if (hasContext) {
                // TODO: handle @context
                XPathAnalysis2.CONSTANT_NEGATIVE_ANALYSIS
            } else if (hasBind) {
                // Use bind analysis
    ...
    protected def findOrCreateBaseAnalysis(startAnalysis: SimpleAnalysis, scope: XBLBindings#Scope, containingModel: Model): XPathAnalysis = {
        val ancestorOrSelf = getAncestorOrSelfBindingAnalysis(startAnalysis, scope, containingModel)
        if (ancestorOrSelf != null)
            ...
            if (containingModel != null) {
                if (containingModel.defaultInstancePrefixedId != null) {
                    ...
                    new XPathAnalysis2(staticState, XPathAnalysis2.buildInstanceString(containingModel.defaultInstancePrefixedId),
    ...
    private def getAncestorOrSelfBindingAnalysis(startAnalysis: SimpleAnalysis, scope: XBLBindings#Scope, containingModel: Model): XPathAnalysis = {
        var currentAnalysis  = startAnalysis
        while (currentAnalysis != null) {
            if (currentAnalysis.getBindingAnalysis != null &&
                    XFormsUtils.compareStrings(currentAnalysis.getModelPrefixedId, if (containingModel != null) containingModel.prefixedId else null) &&
                    currentAnalysis.scope == scope)
    ...
    def analyzeXPath(staticState: XFormsStaticState, baseAnalysis: XPathAnalysis, prefixedId: String, xpathString: String): XPathAnalysis = {
        return new XPathAnalysis2(staticState, xpathString, staticState.getMetadata.getNamespaceMapping(prefixedId),
    ...
    def getInScopeContexts: JMap[String, SimpleAnalysis] = {
        val result = new JHashMap[String, SimpleAnalysis]

        var currentParent = this
        while (currentParent != null) {
            result.put(currentParent.prefixedId, currentParent)
...
object SimpleAnalysis {
    ...
    protected def getAncestorAnalysisInScope(parentAnalysis: SimpleAnalysis, scope: XBLBindings#Scope): SimpleAnalysis = {
        var currentControlAnalysis = parentAnalysis
        while (currentControlAnalysis != null) {
            if (currentControlAnalysis.scope == scope) {
