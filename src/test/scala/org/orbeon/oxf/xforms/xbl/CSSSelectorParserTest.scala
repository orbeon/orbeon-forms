...
class CSSSelectorParserTest extends AssertionsForJUnit {
            ...
            List(
                Selector(
                    ElementWithFiltersSelector(
                        ...
                        List(
                            AttributeFilter(None, "appearance", Some(("=", "minimal"))),
    ...
    @Test def selectors(): Unit = {
        val Expected = Seq(
            ...
            "E[foo=\"bar\"]"                                    → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"E")),List(AttributeFilter(None,"foo",Some(("=","bar"))))),Nil),
            "E[foo~=\"bar\"]"                                   → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"E")),List(AttributeFilter(None,"foo",Some(("~=","bar"))))),Nil),
            "E[foo^=\"bar\"]"                                   → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"E")),List(AttributeFilter(None,"foo",Some(("^=","bar"))))),Nil),
            "E[foo$=\"bar\"]"                                   → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"E")),List(AttributeFilter(None,"foo",Some(("$=","bar"))))),Nil),
            "E[foo*=\"bar\"]"                                   → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"E")),List(AttributeFilter(None,"foo",Some(("*=","bar"))))),Nil),
            "E[foo|=\"en\"]"                                    → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"E")),List(AttributeFilter(None,"foo",Some(("|=","en"))))),Nil),
            ...
            "*[hreflang|=en]"                                   → Selector(ElementWithFiltersSelector(Some(UniversalSelector(None)),List(AttributeFilter(None,"hreflang",Some(("|=","en"))))),Nil),
            "[hreflang|=en]"                                    → Selector(ElementWithFiltersSelector(None,List(AttributeFilter(None,"hreflang",Some(("|=","en"))))),Nil),
            ...
            "span[class=\"example\"]"                           → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"span")),List(AttributeFilter(None,"class",Some(("=","example"))))),Nil),
            "span[hello=\"Cleveland\"][goodbye=\"Columbus\"]"   → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"span")),List(AttributeFilter(None,"hello",Some(("=","Cleveland"))), AttributeFilter(None,"goodbye",Some(("=","Columbus"))))),Nil),
            "a[rel~=\"copyright\"]"                             → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"a")),List(AttributeFilter(None,"rel",Some(("~=","copyright"))))),Nil),
            "a[href=\"http://www.w3.org/\"]"                    → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"a")),List(AttributeFilter(None,"href",Some(("=","http://www.w3.org/"))))),Nil),
            "a[hreflang=fr]"                                    → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"a")),List(AttributeFilter(None,"hreflang",Some(("=","fr"))))),Nil),
            "a[hreflang|=\"en\"]"                               → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"a")),List(AttributeFilter(None,"hreflang",Some(("|=","en"))))),Nil),
//            "a[foo|bar|=\"en\"]" → , // FIXME: unable to parse this yet
            "DIALOGUE[character=romeo]"                         → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"DIALOGUE")),List(AttributeFilter(None,"character",Some(("=","romeo"))))),Nil),
            "DIALOGUE[character=juliet]"                        → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"DIALOGUE")),List(AttributeFilter(None,"character",Some(("=","juliet"))))),Nil),
            "object[type^=\"image/\"]"                          → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"object")),List(AttributeFilter(None,"type",Some(("^=","image/"))))),Nil),
            "a[href$=\".html\"]"                                → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"a")),List(AttributeFilter(None,"href",Some(("$=",".html"))))),Nil),
            "p[title*=\"hello\"]"                               → Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"p")),List(AttributeFilter(None,"title",Some(("*=","hello"))))),Nil),
