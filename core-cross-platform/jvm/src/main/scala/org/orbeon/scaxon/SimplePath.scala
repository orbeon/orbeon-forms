...
//                    val test = new NameTest(nodeKind, fingerprint, pool)
  ...
  implicit class NodeInfoOps(private val nodeInfo: NodeInfo) extends AnyVal {
    ...
    def /@(test: Test): NodeColl = find(Axis.ATTRIBUTE, test)

    def namespaceNodes: NodeColl = find(Axis.NAMESPACE, AnyTest)
...
//        def //@(test: Test): Seq[NodeInfo] = find(Axis.DESCENDANT, test)
    ...
    def ancestor(test: Test): NodeColl = find(Axis.ANCESTOR, test)
    def ancestorOrSelf (test: Test): NodeColl = find(Axis.ANCESTOR_OR_SELF, test)
    def descendant(test: Test): NodeColl = find(Axis.DESCENDANT, test)
    def descendantOrSelf(test: Test): NodeColl = find(Axis.DESCENDANT_OR_SELF, test)

    def preceding(test: Test): NodeColl = find(Axis.PRECEDING, test) // TODO: use Type/NODE?
    def following(test: Test): NodeColl = find(Axis.FOLLOWING, test) // TODO: use Type/NODE?

    def precedingSibling(test: Test): NodeColl = find(Axis.PRECEDING_SIBLING, test) // TODO: use Type/NODE?
    def followingSibling(test: Test): NodeColl = find(Axis.FOLLOWING_SIBLING, test) // TODO: use Type/NODE?
    ...
    private def find(axisNumber: Byte, test: Test): NodeColl =
