...
import org.orbeon.oxf.xml.XMLUtils.ParserConfiguration._
...
class FilterReceiverTest extends ResourceManagerTestBase with AssertionsForJUnit {
    ...
    @Test def resultingEvents() {
        ...
        XMLUtils.urlToSAX("oxf:/org/orbeon/oxf/fb/form-with-metadata.xhtml", metadataFilter, XINCLUDE_ONLY, false)
        ...
        val expected = Seq(
            StartDocument,
                ...
                StartElement("metadata", Atts(Nil)),
                    ...
                    StartElement("title", Atts(Seq(XMLLang → "en"))),
                    EndElement("title"),
                    StartElement("description", Atts(Seq(XMLLang → "en"))),
                    EndElement("description"),
                    StartElement("title", Atts(Seq(XMLLang → "fr"))),
                    EndElement("title"),
                    StartElement("description", Atts(Seq(XMLLang → "fr"))),
                    EndElement("description"),
                    StartElement("permissions", Atts(Nil)),
                        StartElement("permission", Atts(Seq(Operations → "read update delete"))),
