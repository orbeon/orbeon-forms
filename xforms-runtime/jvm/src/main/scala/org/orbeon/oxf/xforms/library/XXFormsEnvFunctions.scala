...
trait XXFormsEnvFunctions extends OrbeonFunctionLibrary {
  ...
  Namespace(XXFormsEnvFunctionsNS) {
    ...
    Fun(ValidationFunctionNames.UploadMaxSizeAggregatePerControl, classOf[UploadMaxSizeAggregatePerControlValidation], op = 0, min = 1, BOOLEAN, EXACTLY_ONE,
      Arg(INTEGER, ALLOWS_ZERO_OR_ONE)
    )
    ...
    Fun(ValidationFunctionNames.UploadMinFilesPerControl, classOf[UploadMinFilesPerControlValidation], op = 0, min = 1, BOOLEAN, EXACTLY_ONE,
      Arg(INTEGER, ALLOWS_ZERO_OR_ONE)
    )
