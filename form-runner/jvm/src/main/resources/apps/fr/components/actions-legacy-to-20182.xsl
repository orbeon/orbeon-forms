...
<xsl:stylesheet
    ...
    <xsl:template match="xf:action[ends-with(@id, '-binding')]">
        ...
        <xsl:choose>
            <xsl:when test="../fr:action[@version = '2018.2'][@name = $action-name]">
                <!-- Already in 2018.2 format; keep legacy action for annotate-design-time.xsl to handle -->
                <xsl:copy-of select="."/>
            </xsl:when>
            <xsl:otherwise>
            ...
            <xsl:variable name="action-body">
                ...
                <xsl:for-each select="
                    ...
                    <xsl:variable name="label" select="fr:var-value((.//xf:var[@name = 'item-label'])[1])"/>
                    <xsl:variable name="value" select="fr:var-value((.//xf:var[@name = 'item-value'])[1])"/>
                    <xsl:variable name="hint" select="if (exists(.//xf:var[@name = 'item-hint'])) then fr:var-value((.//xf:var[@name = 'item-hint'])[1]) else ''"/>
