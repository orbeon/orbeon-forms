...
<p:config xmlns:p="http://www.orbeon.com/oxf/pipeline"
    ...
    <p:processor name="oxf:request">
        <p:input name="config">
            <config>
                <include>/request/parameters/parameter[name = ('state-abbreviation', 'city', 'zip', 'max')]</include>
            </config>
        </p:input>
            ...
            <zips xsl:version="2.0">
                ...
                <xsl:variable name="zips"               as="element(zip)*"
                    select="/zips/zip[
                        ...
                        (p:is-blank($city) or city = $city)      and
                        (p:is-blank($zip)  or starts-with(code, $zip))
                    ]"/>
