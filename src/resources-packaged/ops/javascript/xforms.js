...
ORBEON.xforms.Controls = {
    ...
    _getControlLabel: function(control, className) {
        ...
        if (ORBEON.util.Utils.getProperty(NEW_XHTML_LAYOUT_PROPERTY)) {
            var labels = YAHOO.util.Dom.getElementsByClassName("xforms-label", "label", control);
            if (labels.length > 0) return labels[0];
        }
        // If old layout, or we couldn't find the label, look by ID
        var labelId = ORBEON.util.Utils.appendToEffectiveId(control.id, className);
        return YAHOO.util.Dom.get(labelId);
    },
    ...
    _setMessage: function(control, className, message) {
        ...
        if (label != null) {
            ...
            var helpImage = ORBEON.xforms.Controls._getControlLabel(control, "$$i");
            if (message == "") {
    ...
    getLabelMessage: function(control) {
        if (ORBEON.util.Dom.hasClass(control, "xforms-trigger")
            ...
            var labelElement = ORBEON.xforms.Controls._getControlLabel(control, "$$l");
    ...
    setLabelMessage: function(control, message) {
        if (ORBEON.util.Dom.hasClass(control, "xforms-trigger")
            ...
            ORBEON.xforms.Controls._setMessage(control, "$$l", message);
        }
    },

    getHelpMessage: function(control) {
        var helpElement = ORBEON.xforms.Controls._getControlLabel(control, "$$p");
    ...
    setHelpMessage: function(control, message) {
        ...
        ORBEON.xforms.Controls._setMessage(control, "$$p", message);
    ...
    setValid: function(control, newValid) {
        ...
        var alertElement = ORBEON.xforms.Controls._getControlLabel(control, "$$a");
    ...
    setRelevant: function(control, isRelevant) {
        ...
        if (ORBEON.util.Dom.hasClass(control, "xforms-group-begin-end")) {
            ...
            var elementsToUpdate = [ control,
                ORBEON.xforms.Controls._getControlLabel(control, "$$l"),
                ORBEON.xforms.Controls._getControlLabel(control, "$$a")
            ];
            // Also show help if message is not empty
            if (!isRelevant || (isRelevant && ORBEON.xforms.Controls.getHelpMessage(control) != "")) {
                elementsToUpdate.push(ORBEON.xforms.Controls._getControlLabel(control, "$$p"));
                elementsToUpdate.push(ORBEON.xforms.Controls._getControlLabel(control, "$$i"));
            }
            // Also show hint if message is not empty
            if (!isRelevant || (isRelevant && ORBEON.xforms.Controls.getHintMessage(control) != ""))
                elementsToUpdate.push(ORBEON.xforms.Controls._getControlLabel(control, "$$t"));
    ...
    setAlertMessage: function(control, message) {
        ORBEON.xforms.Controls._setMessage(control, "$$a", message);
    ...
    getHintMessage: function(control) {
        if (ORBEON.util.Dom.hasClass(control, "xforms-trigger")) {
            ...
            var hintElement = ORBEON.xforms.Controls._getControlLabel(control, "$$t");
    ...
    setHintMessage: function(control, message) {
        if (ORBEON.util.Dom.hasClass(control, "xforms-trigger")) {
            ...
            ORBEON.xforms.Controls._setMessage(control, "$$t", message);
        }
            ...
            if (targetControlElement != null) {
                ...
                if (!ORBEON.util.Dom.hasClass(targetControlElement, "xforms-visited")) {
                    ...
                    function afterAjaxResponseAfterBlur() {
                        ...
                        var alertLabel = ORBEON.xforms.Controls._getControlLabel(targetControlElement, "$$a");
                                    ...
                                    for (var j = 0; j < controlElementslength; j++) {
                                        ...
                                        if (!isStaticReadonly && staticReadonly == "true") {
                                            if (isLeafControl) {
                                                ...
                                                // Remove alert
                                                var alertElement = ORBEON.xforms.Controls._getControlLabel(newDocumentElement, "$$a");
                                                if (alertElement != null)
                                                    parentElement.removeChild(alertElement);
                                                // Remove hint
                                                var hintLabel = ORBEON.xforms.Controls._getControlLabel(newDocumentElement, "$$t");
                                                if (hintLabel != null)
