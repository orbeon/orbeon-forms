...
object XFormsFunctionLibrary extends OrbeonFunctionLibrary
    ...
    Fun("last", classOf[Last], 0, 0, 0, INTEGER, EXACTLY_ONE)

    // Forward these to our own implementation so we can handle PathMap
    Fun("count", classOf[org.orbeon.oxf.xforms.function.Aggregate], Aggregate.COUNT, 1, 1, INTEGER, EXACTLY_ONE,
        Arg(Type.ITEM_TYPE, ALLOWS_ZERO_OR_MORE, IntegerValue.ZERO)
    )

    Fun("avg", classOf[org.orbeon.oxf.xforms.function.Aggregate], Aggregate.AVG, 1, 1, ANY_ATOMIC, ALLOWS_ZERO_OR_ONE,
        Arg(ANY_ATOMIC, ALLOWS_ZERO_OR_MORE, EmptySequence.getInstance())
    )

    Fun("sum", classOf[org.orbeon.oxf.xforms.function.Aggregate], Aggregate.SUM, 1, 2, ANY_ATOMIC, ALLOWS_ZERO_OR_ONE,
