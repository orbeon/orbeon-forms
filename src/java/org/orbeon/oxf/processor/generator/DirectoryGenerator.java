...
/**
 * This generator allows scanning a filesystem hierarchy, and retrieve information for each file. For now, the
 * implementation is based on a modified version of the ant directory scanner.
 *
 * Ideas for improvements:
 *
 * o componentize so that file identification and enhanced file information (such as images) can be plugged
            ...
            public void readImpl(PipelineContext context, ContentHandler contentHandler) {
                ...
                final Config config = (Config) readCacheInputAsObject(context, getInputByName(INPUT_CONFIG), new CacheableInputReader() {
                    public Object read(PipelineContext context, ProcessorInput input) {
                        ...
                        String baseDirectory = XPathUtils.selectStringValueNormalize(configNode, "/config/base-directory").trim();
                        config.setBaseDirectory(baseDirectory);
                        for (Iterator i = XPathUtils.selectIterator(configNode, "/config/include"); i.hasNext();) {
                            Node node = (Node) i.next();
                            String value = XPathUtils.selectStringValueNormalize(node, ".");
                            if (value != null)
                        ...
                        boolean caseSensitive = ProcessorUtils.selectBooleanValue(configNode, "/config/case-sensitive", DEFAULT_CASE_SENSITIVE);
                        config.setCaseSensitive(caseSensitive);
                        boolean defaultExcludes = ProcessorUtils.selectBooleanValue(configNode, "/config/default-excludes", DEFAULT_DEFAULT_EXCLUDES);
                        config.setDefaultExcludes(defaultExcludes);

                        boolean basicInfo = ProcessorUtils.selectBooleanValue(configNode, "/config/image-metadata/basic-info", DEFAULT_BASIC_INFO);
                        config.setBasicInfo(basicInfo);
                        boolean exifInfo = ProcessorUtils.selectBooleanValue(configNode, "/config/image-metadata/exif-info", DEFAULT_EXIF_INFO);
                        config.setExifInfo(exifInfo);
                        boolean iptcInfo = ProcessorUtils.selectBooleanValue(configNode, "/config/image-metadata/iptc-info", DEFAULT_IPTC_INFO);
                ...
                try {
                    ...
                    helper.startElement(ROOT_ELEMENT);
...
//                    }
    ...
    private static void outputMetadata(ContentHandlerHelper helper, Metadata metadata, String elementName) throws SAXException, MetadataException {
