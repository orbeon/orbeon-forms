...
object XFormsInsertAction {
  ...
  def doInsert(
    ...
    collectionToBeUpdated             : util.List[_],
    insertContextNodeInfo             : NodeInfo,
    originItems                       : Option[util.List[Item]],
    insertionIndex                    : Int,
    doClone                           : Boolean,
    doDispatch                        : Boolean,
    ...
    val isEmptyNodesetBinding = collectionToBeUpdated == null || collectionToBeUpdated.size == 0
    ...
    val insertLocationNodeInfo =
      if (isEmptyNodesetBinding) {
        ...
        collectionToBeUpdated.get(insertionIndex - 1).asInstanceOf[NodeInfo]
      }

    // Identify the instance that actually changes
    val modifiedInstanceOpt =
      searchForInstance flatOption
        (containingDocument flatMap (_.instanceForNodeOpt(insertLocationNodeInfo)))
      ...
      originItems match {
        case None =>
          ...
          val singleSourceNode =
            XFormsUtils.getNodeFromNodeInfoConvert(collectionToBeUpdated.get(collectionToBeUpdated.size - 1).asInstanceOf[NodeInfo])
        ...
        case Some(originItems) =>
          ...
          for (currentItem <- originItems.asScala.iterator)
    ...
    val clonedNodesIt =
      if (removeInstanceDataFromClonedNodes) {
        ...
        for (clonedNodeTemp <- clonedNodesTmp)
          yield clonedNodeTemp match {
            ...
            case _: Document =>
              val clonedNodeTempRootElement = clonedNodeTemp.getDocument.getRootElement
              if (clonedNodeTempRootElement eq null) {
                // Can be null in rare cases of documents without root element
                // we support having a null node further below, so set this to null
                null
              } else {
                if (applyDefaults)
                ...
                clonedNodeTempRootElement.detach()
              }
            case _ =>
              clonedNodeTemp.detach()
          }
        ...
        for (clonedNodeTemp <- clonedNodesTmp)
          yield clonedNodeTemp.detach()
      }

    val clonedNodes = clonedNodesIt.toList
    ...
    val (insertLocationIndexWithinParentBeforeUpdate, insertedNodes, beforeAfterInto) =
      if (isEmptyNodesetBinding) {
        ...
        val insertLocationNodeDocument = insertLocationNode.getDocument

        val (insertedNodes, beforeAfterInto) =
          if (insertLocationNodeDocument != null && (insertLocationNodeDocument.getRootElement eq insertLocationNode)) {
            ...
            (
              ...
              positionAttribute // TODO: ideally normalize to "into document node"?
            )
  ...
  private def doInsert(
    ...
    insertionNode match {
      ...
      case _ =>
        throw new OXFException(s"Unsupported insertion node type: `${insertionNode.getClass.getName}`")
    }
  }
}
...
class XFormsInsertAction extends XFormsAction {
  ...
  override def execute(
    ...
    val atAttribute = actionElement.attributeValue("at")
    val originAttributeOpt = actionElement.attributeValueOpt("origin")
    val contextAttribute = actionElement.attributeValue(XFormsNames.CONTEXT_QNAME)

    // Extension: allow position to be an AVT
    val resolvedPositionAttribute = Option(actionInterpreter.resolveAVT(actionElement, "position"))

    // Extension: xxf:default="true" AVT requires that recalculate apply default values on the inserted nodes.
    val setRequireDefaultValues = "true" == actionInterpreter.resolveAVT(actionElement, XFormsNames.XXFORMS_DEFAULTS_QNAME)

    // "2. The Node Set Binding node-set is determined."
    val currentBindingContext = contextStack.getCurrentBindingContext

    val collectionToBeUpdated =
      if (currentBindingContext.newBind)
        currentBindingContext.nodeset
      else
        java.util.Collections.emptyList[Item]

    val isEmptyNodesetBinding = collectionToBeUpdated.size == 0
    ...
    val insertContextItem =
      if (hasOverriddenContext) {
        ...
        if (overriddenContext == null || !overriddenContext.isInstanceOf[NodeInfo]) {
          if (indentedLogger.isDebugEnabled)
    ...
    val originItemsOpt =
      originAttributeOpt map { originAttribute =>
        ...
        val originObjects =
          actionInterpreter.evaluateKeepItemsJava(actionElement, Collections.singletonList(insertContextItem), 1, originAttribute)
    ...
    val insertionIndex =
      if (isEmptyNodesetBinding) {
        ...
        val insertionIndexString =
          actionInterpreter.evaluateAsString(actionElement, collectionToBeUpdated, 1, "round(" + atAttribute + ")")
        ...
        insertionIndex min collectionToBeUpdated.size max 1
      }

    val normalizedPosition =
      resolvedPositionAttribute match {
        case Some(p @ ("after" | "before")) =>
          p
        case None =>
          "after"
        case _ =>
          if (indentedLogger.isInfoEnabled)
            indentedLogger.logWarning(
              ...
              "value", resolvedPositionAttribute.orNull
            )
          "after"
      }
    ...
    XFormsInsertAction.doInsert(
      ...
      indentedLogger                    = indentedLogger,
      positionAttribute                 = normalizedPosition,
      collectionToBeUpdated             = collectionToBeUpdated,
      insertContextNodeInfo             = insertContextItem.asInstanceOf[NodeInfo],
      originItems                       = originItemsOpt,
