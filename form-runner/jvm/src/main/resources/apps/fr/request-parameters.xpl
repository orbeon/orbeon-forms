...
<p:config xmlns:p="http://www.orbeon.com/oxf/pipeline"
    ...
    <p:processor name="oxf:regexp">
        <p:input name="config"><config>/fr/(service/)?([^/]+)/([^/]+)/(new|edit|view|pdf|test-pdf|tiff|excel-export|controls|email|validate|import|schema|duplicate|attachments|publish|compile)(/([^/]+))?(/([0-9A-Za-z\-]+)(?:/[^/]+)?\.(?:pdf|tiff))?</config></p:input>
