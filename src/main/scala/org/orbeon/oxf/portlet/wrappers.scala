...
trait RequestPrependHeaders extends PortletRequestWrapper {
    ...
    override def getPropertyNames = (headersToPrepend.keysIterator ++ (super.getPropertyNames.asScala filterNot headersToPrepend.keySet)).asJavaEnumeration

    private def addedHeaderOption(name: String) = headersToPrepend.get(name) filter (_.nonEmpty) map (_(0))

    override def getProperty(name: String)      = addedHeaderOption(name) getOrElse super.getProperty(name)
    override def getProperties(name: String)    = headersToPrepend.get(name) map (_.iterator.asJavaEnumeration) getOrElse super.getProperties(name)
}
