...
class CSSSelectorParserTest extends AssertionsForJUnit {
      ...
      List(
        Selector(
          ElementWithFiltersSelector(
            ...
            List(
              AttributeFilter(None, "appearance", Some(AttributePredicate("=", "minimal"))),
  ...
  @Test def selectors(): Unit = {
    val Expected = Seq(
      ...
      "E[foo]"                                            -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"E")),List(AttributeFilter(None,"foo",None))),Nil),
      "E[foo=\"bar\"]"                                    -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"E")),List(AttributeFilter(None,"foo",Some(AttributePredicate("=","bar"))))),Nil),
      "E[foo~=\"bar\"]"                                   -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"E")),List(AttributeFilter(None,"foo",Some(AttributePredicate("~=","bar"))))),Nil),
      "E[foo^=\"bar\"]"                                   -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"E")),List(AttributeFilter(None,"foo",Some(AttributePredicate("^=","bar"))))),Nil),
      "E[foo$=\"bar\"]"                                   -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"E")),List(AttributeFilter(None,"foo",Some(AttributePredicate("$=","bar"))))),Nil),
      "E[foo*=\"bar\"]"                                   -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"E")),List(AttributeFilter(None,"foo",Some(AttributePredicate("*=","bar"))))),Nil),
      "E[foo|=\"en\"]"                                    -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"E")),List(AttributeFilter(None,"foo",Some(AttributePredicate("|=","en"))))),Nil),
      ...
      "*[hreflang|=en]"                                   -> Selector(ElementWithFiltersSelector(Some(UniversalSelector(None)),List(AttributeFilter(None,"hreflang",Some(AttributePredicate("|=","en"))))),Nil),
      "[hreflang|=en]"                                    -> Selector(ElementWithFiltersSelector(None,List(AttributeFilter(None,"hreflang",Some(AttributePredicate("|=","en"))))),Nil),
      ...
      "h1[title]"                                         -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"h1")),List(AttributeFilter(None,"title",None))),Nil),
      "span[class=\"example\"]"                           -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"span")),List(AttributeFilter(None,"class",Some(AttributePredicate("=","example"))))),Nil),
      "span[hello=\"Cleveland\"][goodbye=\"Columbus\"]"   -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"span")),List(AttributeFilter(None,"hello",Some(AttributePredicate("=","Cleveland"))), AttributeFilter(None,"goodbye",Some(AttributePredicate("=","Columbus"))))),Nil),
      "a[rel~=\"copyright\"]"                             -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"a")),List(AttributeFilter(None,"rel",Some(AttributePredicate("~=","copyright"))))),Nil),
      "a[href=\"http://www.w3.org/\"]"                    -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"a")),List(AttributeFilter(None,"href",Some(AttributePredicate("=","http://www.w3.org/"))))),Nil),
      "a[hreflang=fr]"                                    -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"a")),List(AttributeFilter(None,"hreflang",Some(AttributePredicate("=","fr"))))),Nil),
      "a[hreflang|=\"en\"]"                               -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"a")),List(AttributeFilter(None,"hreflang",Some(AttributePredicate("|=","en"))))),Nil),
//            "a[foo|bar|=\"en\"]" -> , // FIXME: unable to parse this yet
      "DIALOGUE[character=romeo]"                         -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"DIALOGUE")),List(AttributeFilter(None,"character",Some(AttributePredicate("=","romeo"))))),Nil),
      "DIALOGUE[character=juliet]"                        -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"DIALOGUE")),List(AttributeFilter(None,"character",Some(AttributePredicate("=","juliet"))))),Nil),
      "object[type^=\"image/\"]"                          -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"object")),List(AttributeFilter(None,"type",Some(AttributePredicate("^=","image/"))))),Nil),
      "a[href$=\".html\"]"                                -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"a")),List(AttributeFilter(None,"href",Some(AttributePredicate("$=",".html"))))),Nil),
      "p[title*=\"hello\"]"                               -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"p")),List(AttributeFilter(None,"title",Some(AttributePredicate("*=","hello"))))),Nil),
...
//            ":nth-child(10n+-1)" -> , // FIXME: should be invalid!
      ...
      "button:not([DISABLED])"                            -> Selector(ElementWithFiltersSelector(Some(TypeSelector(None,"button")),List(NegationFilter(AttributeFilter(None,"DISABLED",None)))),Nil),
