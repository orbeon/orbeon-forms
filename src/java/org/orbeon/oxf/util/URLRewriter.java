        ...
        try {
            final String baseURLString;
            {
                ...
                if (rewriteMode == ExternalContext.Response.REWRITE_MODE_ABSOLUTE) {
                    final URI absoluteBaseURI = new URI(request.getRequestURL()).resolve("/");
                    _baseURLString = absoluteBaseURI.toString();
                } else {
                    _baseURLString = "";
                }
                // Remove trailing slash if any
                if (_baseURLString.endsWith("/"))
                    _baseURLString = _baseURLString.substring(0, _baseURLString.length() - 1);
