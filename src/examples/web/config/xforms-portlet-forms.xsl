...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="reference-xform-instance" as="element()?">
        <xsl:copy-of select="(/xhtml:html/xhtml:body//xhtml:form/xhtml:input[ @type='hidden' and @name='$instance' and not(@value='')])[1]"/>
    </xsl:variable>
    ...
    <xsl:template match="*[generate-id() = $forms-ids]">
        <xsl:copy>
            ...
            <xsl:if test="$reference-xform-instance">
                <xhtml:input type="hidden" name="$instance" value="{$reference-xform-instance/@value}"/> 
            </xsl:if>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="*[generate-id() = $anchor-ids]" mode="#all">
        <xsl:copy>
            ...
            <xsl:if test="$reference-xform-instance and contains(@href, '$portlet$')"><!-- Don't add instance if we are going out of the portal anyway -->
                <xsl:variable name="params" select="concat('$instance=', escape-uri(normalize-space($reference-xform-instance/@value), true()))"/>
