...
ORBEON.util = {
    ...
    Dom: {
        ...
        /**
         * Legacy method, which should never be used. This method is deprecated and is up for deletion.
         */
         ...
         */
        safeGetElementById: function(controlId) {
...
ORBEON.util.Utils = {
    ...
    findRepeatDelimiter: function(repeatId, index) {
        ...
        var beginElement = YAHOO.util.Dom.get(beginElementId);
...
ORBEON.xforms.Controls = {
    ...
    getCurrentValue: function(control) {
        if (YAHOO.util.Dom.hasClass(control, "xforms-type-time")) {
            ...
            // Select/Select tree
            var oneValue = YAHOO.util.Dom.hasClass(control, "xforms-select1-appearance-xxforms-tree");
    ...
    setCurrentValue: function(control, newControlValue, attribute1, attribute2, attribute3, attribute4) {
        ...
        } else if (YAHOO.util.Dom.hasClass(control, "xforms-select1-appearance-xxforms-tree")) {
            // Select1 tree
    ...
    showDialog: function(controlId, neighbor) {
        var divElement = YAHOO.util.Dom.get(controlId);
        ...
        if (ORBEON.xforms.Globals.currentFocusControlId != null) {
            var focusedElement = YAHOO.util.Dom.get(ORBEON.xforms.Globals.currentFocusControlId);
...
ORBEON.xforms.Events = {
    ...
    focus: function(event) {
        ...
        if (!ORBEON.xforms.Globals.maskFocusEvents) {
            ...
            var currentFocusControlElement = ORBEON.xforms.Globals.currentFocusControlId != null ? YAHOO.util.Dom.get(ORBEON.xforms.Globals.currentFocusControlId) : null;
    ...
    _showToolTip: function(tooltipForControl, control, target, toolTipSuffix, message, delay, event) {
        ...
        if (tooltipForControl[control.id] == null) {
            if (message != "") {
                ...
                var context = YAHOO.util.Dom.get(target.id);
...
//        }
    ...
    sliderValueChange: function(offset) {
        ...
        var rangeControl = YAHOO.util.Dom.get(this.id);
        if (ORBEON.util.Utils.isNewXHTMLLayout())
    ...
    dialogClose: function(type, args, me) {
        if (! ORBEON.xforms.Globals.maskDialogCloseEvents) {
            var dialogId = me;
            var dialog = YAHOO.util.Dom.get(dialogId);
    ...
    sendHeartBeatIfNeeded: function(heartBeatDelay) {
        ...
        if ((currentTime - ORBEON.xforms.Globals.lastEventSentTime) >= heartBeatDelay) {
            var heartBeatDiv = YAHOO.util.Dom.get("xforms-heartbeat");
...
ORBEON.widgets.YUICalendar = function() {
    ...
    return {
        ...
        click: function(event, target) {
            ...
            if (calendarDiv == null) {
                calendarDiv = YAHOO.util.Dom.get("orbeon-calendar-div");
...
ORBEON.widgets.RTE = function() {
    ...
    function sendChangeToServer(controlID) {
        var event = new ORBEON.xforms.Server.Event(null, controlID, null,
                ORBEON.widgets.RTE.getValue(YAHOO.util.Dom.get(controlID)), "xxforms-value-change-with-focus-change");
    ...
    function changeEvent(controlID) {
        ...
        if (currentFocusControlId != controlID) {
            ...
            if (rteEditors[currentFocusControlId] != null) {
                ORBEON.xforms.Events.blur({ target: YAHOO.util.Dom.get(currentFocusControlId) });
            }
            // Send focus to current control
            ORBEON.xforms.Events.focus({ target: YAHOO.util.Dom.get(controlID) });
        }

        // Simulate keyup
        ORBEON.xforms.Events.keydown({ target: YAHOO.util.Dom.get(currentFocusControlId) });
        ORBEON.xforms.Events.keyup({ target: YAHOO.util.Dom.get(currentFocusControlId) });
    }
    ...
    var PUBLIC = {
        ...
        focusOnAnyFormControl: function(control) {
            ...
            if (rteEditors[control.id] == null && rteEditors[currentFocusControlId] != null) {
                ...
                ORBEON.xforms.Events.change({ target: YAHOO.util.Dom.get(currentFocusControlId) });
                ORBEON.xforms.Events.blur({ target: YAHOO.util.Dom.get(currentFocusControlId) });
            }
        },
...
ORBEON.xforms.Init = {
    ...
    document: function() {
        ...
        for (var formIndex = 0; formIndex < document.forms.length; formIndex++) {
            ...
            if (YAHOO.util.Dom.hasClass(form, "xforms-form")) {
                ...
                YAHOO.util.Dom.removeClass(form, "xforms-initially-hidden");
                            ...
                            for (var controlAppearance in controlAppearances) {
                                ...
                                if (initFunction) {
                                    ...
                                    for (var controlIndex = 0; controlIndex < controlIds.length; controlIndex++) {
                                        var control = YAHOO.util.Dom.get(controlIds[controlIndex]);
    ...
    _tree: function(treeDiv) {
        ...
        var allowMultipleSelection = YAHOO.util.Dom.hasClass(treeDiv, "xforms-select");
		var showToolTip = YAHOO.util.Dom.hasClass(treeDiv, "xforms-show-tooltip");
        if (ORBEON.util.Utils.isNewXHTMLLayout())
		    ...
		    function addTreeToolTip() {
                ...
                if (nodes != null) {
        			for (var nodeIndex = 0; nodeIndex < nodes.length; nodeIndex++) {
        				...
        				var contentEl = YAHOO.util.Dom.get(node.contentElId);
    ...
    _htmlArea: function (htmlArea) {
        if (ORBEON.util.Properties.htmlEditor.get() == "yui") {
            ...
            if (ORBEON.xforms.Globals.fckEditorLoading) {
                ...
                ORBEON.xforms.Controls.updateHTMLAreaClasses(YAHOO.util.Dom.get(fckEditor.InstanceName));
            }
        }
    },
...
ORBEON.xforms.Server = {
    ...
    Event: function(form, targetId, otherId, value, eventName, bubbles, cancelable, ignoreErrors, showProgress, progressMessage, additionalAttribs) {
        ...
        this.form = YAHOO.lang.isObject(form) ? form
            : YAHOO.lang.isString(targetId) ? ORBEON.xforms.Controls.getForm(YAHOO.util.Dom.get(targetId)) : null;
    ...
    createDelayedServerEvent: function(serverEvents, delay, showProgress, progressMessage, discardable, formID) {
        var timerId = window.setTimeout(function () {
            var event = new ORBEON.xforms.Server.Event(YAHOO.util.Dom.get(formID), null, null,
            ...
            if (ORBEON.util.Properties.clientEventMode.get() == "deferred") {
                ...
                eventLoop: for (var eventIndex = 0; eventIndex < ORBEON.xforms.Globals.eventQueue.length; eventIndex++) {
                    ...
                    if (event.targetId != null && eventsToFilter[event.eventName] == null) {
                        var target = YAHOO.util.Dom.get(event.targetId);
                        if (target == null) {
            ...
            if (foundActivatingEvent) {
                        ...
                        if (eventsToFilter[event.eventName] == null) {
                            if (event.eventName == "xxforms-value-change-with-focus-change") {
                                ...
                                if (seenControlValue[event.targetId] == null) {
                                    ...
                                    if (YAHOO.util.Dom.hasClass(YAHOO.util.Dom.get(event.targetId), "xforms-upload") ||
                ...
                if (ORBEON.xforms.Globals.eventQueue.length > 0) {
                    ...
                    for (var eventIndex = 0; eventIndex < ORBEON.xforms.Globals.eventQueue.length; eventIndex++) {
                        ...
                        if (event.eventName == "DOMActivate") {
                            var eventElement = YAHOO.util.Dom.get(event.targetId);
                            ...
                            case "control-values": {
                                ...
                                for (var j = 0; j < copyRepeatTemplateElementsLength; j++) {
                                    ...
                                    var templateNodes = new Array();
                                    {
                                        ...
                                        var templateRepeatEnd = YAHOO.util.Dom.get("repeat-end-" + repeatId);
                                    ...
                                    var afterInsertionPoint;
                                    {
                                        if (parentIndexes == "") {
                                            ...
                                            var repeatEnd = YAHOO.util.Dom.get("repeat-end-" + repeatId);
                                            ...
                                            var repeatEnd = YAHOO.util.Dom.get("repeat-end-" + ORBEON.util.Utils.appendRepeatSuffix(repeatId, parentIndexes));
                                ...
                                for (var j = 0; j < deleteRepeatTemplateElementsLength; j++) {
                                    ...
                                    var repeatEnd = YAHOO.util.Dom.get("repeat-end-" + ORBEON.util.Utils.appendRepeatSuffix(deleteId, parentIndexes));
                        ...
                        if (ORBEON.util.Utils.getLocalName(actionElement.childNodes[actionIndex]) == "control-values") {
                            ...
                            for (var j = 0; j < itemsetsElement.childNodes.length; j++) {
                                if (ORBEON.util.Utils.getLocalName(itemsetsElement.childNodes[j]) == "itemset") {
                                    ...
                                    var documentElement = YAHOO.util.Dom.get(controlId);
                                    ...
                                    if (YAHOO.util.Dom.hasClass(documentElement, "xforms-select1-open")) {
                                        ...
                                        var template = YAHOO.util.Dom.hasClass(documentElement, "xforms-select")
                                                ? YAHOO.util.Dom.get("xforms-select-full-template")
                                                : YAHOO.util.Dom.get("xforms-select1-full-template");
                    ...
                    for (var actionIndex = 0; actionIndex < actionElement.childNodes.length; actionIndex++) {
                        ...
                        switch (actionName) {
                            ...
                            case "control-values": {
                                ...
                                for (var j = 0; j < controlElementsLength; j++) {
                                    ...
                                    if (documentElement == null) {
                                        documentElement = YAHOO.util.Dom.get("group-begin-" + controlId);
                                    ...
                                    if (type != null && YAHOO.util.Dom.hasClass(documentElement, "xforms-input")) {
                                        ...
                                        YAHOO.util.Dom.removeClass(documentElement, "xforms-type-string");
                                        YAHOO.util.Dom.removeClass(documentElement, "xforms-type-date");
                                        YAHOO.util.Dom.removeClass(documentElement, "xforms-type-time");
                                        YAHOO.util.Dom.removeClass(documentElement, "xforms-type-dateTime");
                                        YAHOO.util.Dom.removeClass(documentElement, "xforms-type-boolean");
                                        ...
                                        if (isStringType) {
                                            ...
                                            var booleanTemplate = YAHOO.util.Dom.get("xforms-select-full-template");
                                ...
                                for (var j = 0; j < attributeElementslength; j++) {
                                    ...
                                    var htmlElement = YAHOO.util.Dom.get(forAttribute);
                                ...
                                for (var j = 0; j < divElementsLength; j++) {
                                    ...
                                    if (yuiDialog == null) {
                                        ...
                                        var caseBegin = YAHOO.util.Dom.get(caseBeginId);
                                        ...
                                        if (visible) {
                                            ...
                                            elementsWithVisibilityChanged[0] = YAHOO.util.Dom.get(controlVisibilityChangeId);
                                        } else {
                            ...
                            case "submission": {
                                ...
                                var requestForm = YAHOO.util.Dom.get(formID);
                                if (replace == "all") {
                                    // Go to another page
...
ORBEON.xforms.Offline = {
    ...
    pageLoad: function() {
        ...
        if (ORBEON.xforms.Offline.hasGears) {
            ...
            if (! ORBEON.xforms.Offline.isOnline) {
                ...
                ORBEON.xforms.Globals.requestForm = YAHOO.util.Dom.get(formID);
    ...
    takeOffline: function(eventResponse, formID, mappings) {
        ...
        */
        ...
        for (var controlIndex = 0; controlIndex < controlKeepValueIDs.length; controlIndex++) {
            ...
            controlValues[controlID] = ORBEON.xforms.Controls.getCurrentValue(YAHOO.util.Dom.get(controlID));
        }
        ...
        if (eventsString != "") {
            ...
            for (var eventIndex = 0; eventIndex < eventsStringArray.length; eventIndex++) {
                ...
                events.push(new ORBEON.xforms.Server.Event(
                    YAHOO.util.Dom.get(eventArray[0]),
                    eventArray[1],
                    eventArray[2],
                    eventArray[3],
                    eventArray[4],
    ...
    loadFormInIframe: function(url, loadListener) {
        ...
        var offlineIframe = YAHOO.util.Dom.get(offlineIframeId);
    ...
    evaluateMIPs: function() {
            ...
            if (getter(control) != value) {
                ...
                if (inherited) {
                    for (var inheritedControlIndex = 0; inheritedControlIndex < mips.relevant.inherited.length; inheritedControlIndex++) {
                        ...
                        var inheritedControl = YAHOO.util.Dom.get(controlID);
                        if (inheritedControl == null) {
                            // If we can't find the control, maybe this is a the ID of a group
                            inheritedControl = YAHOO.util.Dom.get("group-begin-" + controlID);
                        }
        ...
        ORBEON.xforms.Offline._runShowingIndicator(function() {
            ...
            for (var variableName in ORBEON.xforms.Offline.variables) {
                ...
                var variableValue = ORBEON.xforms.Controls.getCurrentValue(YAHOO.util.Dom.get(controlID));
...
function xformsLog(object) {
    var debugDiv = YAHOO.util.Dom.get("xforms-debug");
    if (debugDiv == null) {
