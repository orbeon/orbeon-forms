        ...
        case (ns, value) =>
          CSSSelectorParser.parseSelectors(value) collect {
            ...
            case Selector(ElementWithFiltersSelector(Some(TypeSelector(NsType.Specific(prefix), localname)), Nil),
                List((Combinator.Child, ElementWithFiltersSelector(Some(UniversalSelector(NsType.Default)), Nil)))) =>
            ...
            case Selector(ElementWithFiltersSelector(Some(TypeSelector(NsType.Specific(prefix), localname)),
                List(Filter.Negation(Filter.Attribute(TypeSelector(NsType.Default, attrName), AttributePredicate.Equal(attrValue))))), Nil) =>
              ElementAttributeValueMatcher(ns(prefix) -> localname, attrName, attrValue, negate = true)
            case Selector(ElementWithFiltersSelector(Some(TypeSelector(NsType.Specific(prefix), localname)),
                List(Filter.Attribute(TypeSelector(NsType.Default, attrName), AttributePredicate.Equal(attrValue)))), Nil) =>
