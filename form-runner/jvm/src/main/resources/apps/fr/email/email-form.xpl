...
<p:config xmlns:p="http://www.orbeon.com/oxf/pipeline"
    ...
    <p:processor name="oxf:pipeline">
        ...
        <p:output name="data" id="xhtml-fr-xforms-raw"/>
    </p:processor>

    <!-- Migrate email definition to 2022.1 format -->
    <p:processor name="oxf:unsafe-xslt">
        <p:input  name="data"   href="#xhtml-fr-xforms-raw"/>
        <p:input  name="config" href="email-migrate-form.xsl"/>
    ...
    <p:for-each href="#messages" select="/messages/message" root="sent-messages" id="sent-messages">
        <p:processor name="oxf:identity">
            <p:input name="data" href="current()"/>
            <p:output name="data" id="message"/>
        </p:processor>

        <!-- Remove `fr:uri` on `<part>`, which the email processor doesn't need and want -->
        <p:processor name="oxf:unsafe-xslt">
            <p:input name="data" href="#message"/>
            <p:input name="config">
                <xsl:stylesheet version="2.0">
                    <xsl:import href="oxf:/oxf/xslt/utils/copy.xsl"/>
                    <xsl:template match="@fr:uri"/>
                </xsl:stylesheet>
            </p:input>
            <p:output name="data" id="message-no-uri"/>
        </p:processor>
        ...
        <p:processor name="oxf:pipeline">
            ...
            <p:input name="message-no-uri" href="#message-no-uri"/>
            ...
            <p:input name="config" href="aggregate('root', #message, #parameters-with-version)" transform="oxf:unsafe-xslt">
                <p:config xsl:version="2.0">
                    <!-- Forwarded inputs -->
                    <p:param type="input" name="message"/>
                    <p:param type="input" name="message-no-uri"/>
                    <p:param type="input" name="form-xml"/>
                    <p:param type="input" name="form-pdf"/>
                    <p:param type="input" name="form-tiff"/>
                    <xsl:variable name="parts-with-uri" select="/*/message/body/part[@fr:uri]"/>
                    <!-- Iterate over attachments -->
                    <xsl:for-each select="$parts-with-uri">
                        <xsl:variable name="uri" select="@fr:uri" as="xs:string"/>
                    ...
                    <p:processor name="oxf:email">
                        ...
                        <p:input name="data" href="#message-no-uri"/>
        ...
        <p:processor name="oxf:identity">
            <p:input name="data" href="#message"/>
            <p:output name="data" ref="sent-messages"/>
        </p:processor>
    </p:for-each>

    <!-- Make sure we read the output of the previous p:for-each or else the emails won't be sent -->
    <p:processor name="oxf:null-serializer">
        <p:input name="data" href="#sent-messages"/>
    </p:processor>
