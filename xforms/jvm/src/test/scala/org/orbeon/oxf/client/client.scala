...
trait OrbeonFormsOps extends WebBrowser with Matchers {
  ...
  implicit val patienceConfig: PatienceConfig = PatienceConfig(timeout = scaled(OrbeonClientBase.DefaultTimeout), interval = scaled(Span(100, Millis)))
  ...
  def waitForAjaxResponse() = eventually {
    assert(executeScript("return ! (ORBEON.xforms.AjaxServer.hasEventsToProcess()))").asInstanceOf[Boolean])
  }
...
trait FormBuilderOps extends FormRunnerOps {
  ...
  object Builder {
    ...
    def countCurrentGridRows =
      executeScript("return ORBEON.jQuery('.fb-selected').closest('.fr-grid').find('.fr-grid-tr').size()").asInstanceOf[Long]
