    ...
    if (! deletingDataDraft && ! forceDelete) {
      ...
      val insertSql =
        s"""|INSERT INTO $table
            ...
            |     VALUES ( $colValues )
            |""".stripMargin
      val autoGeneratedColumns = if (table == "orbeon_form_data") Array("id") else Array.empty[String]

      useAndClose(connection.prepareStatement(insertSql, autoGeneratedColumns)) { ps =>
        ...
        val idOpt         = generatedKeys.next().option(generatedKeys.getInt(1))
