...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="reference-xform-instance" as="element()*">
        <xsl:copy-of select="(/xhtml:html/xhtml:body//xhtml:form/xhtml:input[ @type='hidden' and (@name='$instance' or @name='$static-state' or @name='$dynamic-state') and not(@value='')])[1]"/>
    </xsl:variable>
    ...
    <xsl:template match="*[generate-id() = $forms-ids]">
        <xsl:copy>
            ...
            <xsl:for-each select="$reference-xform-instance">
    ...
    <xsl:template match="*[generate-id() = $anchor-ids]" mode="#all">
        <xsl:copy>
            ...
            <xsl:if test="$reference-xform-instance and contains(@href, '$portlet$')"><!-- Don't add instance if we are going out of the portal anyway -->
                <xsl:variable name="params" select="string-join(for $i in $reference-xform-instance return concat($i/@name, '=', escape-uri(normalize-space($i/@value), true())), '&amp;')"/>
