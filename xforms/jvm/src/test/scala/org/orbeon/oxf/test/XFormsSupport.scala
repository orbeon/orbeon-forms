...
trait XFormsSupport extends MockitoSugar {
  ...
  private def mockActionInterpreter(doc: XFormsContainingDocument) = {
    ...
    Mockito when actionInterpreter.resolveObject(ArgumentMatchers.any[ActionTrait], ArgumentMatchers.anyString) thenAnswer
      (
        (invocation: InvocationOnMock) => {
          val targetStaticOrAbsoluteId = invocation.getArguments()(1).asInstanceOf[String]
          doc.resolveObjectByIdOpt(Constants.DocumentId, targetStaticOrAbsoluteId, contextItemOpt = None).orNull
        }
      )
  ...
  def setControlValueWithEventSearchNested(controlEffectiveId: String, value: String): Unit =
    Option(getObject(controlEffectiveId)) match {
      case Some(c: XFormsControl) =>
