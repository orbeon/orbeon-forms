...
ORBEON.xforms.Controls = {
     ...
     */
    _getControlLHHA: function(control, className) {
        ...
        // For new layout, try to look for element under the control element
        if (ORBEON.util.Utils.isNewXHTMLLayout()) {
            var lhhaElements = YAHOO.util.Dom.getElementsByClassName("xforms-" + className, null, control);
            if (lhhaElements.length > 0) return lhhaElements[0];
        }
        // If old layout, or we couldn't find the element, look by ID
        var lhhaElementId = ORBEON.util.Utils.appendToEffectiveId(control.id, classNameToId[className]);
            ...
            if (targetControlElement != null) {
                ...
                if (!ORBEON.util.Dom.hasClass(targetControlElement, "xforms-visited")) {
                    ...
                    function afterAjaxResponseAfterBlur() {
                        ...
                        var alertElement = ORBEON.xforms.Controls._getControlLHHA(targetControlElement, "alert");
    ...
    mouseover: function(event) {
        ...
        if (target != null) {
            ...
            if (ORBEON.util.Utils.getProperty(HELP_TOOLTIP_PROPERTY)
                ...
                // Get control
                var control = ORBEON.util.Dom.getElementById(helpElement.htmlFor);
                if (control) {
