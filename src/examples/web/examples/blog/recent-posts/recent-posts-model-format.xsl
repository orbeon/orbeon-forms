    ...
    <xsl:template match="/">
        ...
        <recent-posts>
            ...
            <feeds>
                <feed>
                    <name>All</name>
                    <link><xsl:value-of select="local:blog-feed-path(doc('input:instance')/*/username, $blog/blog-id, 'rss20', ())"/></link>
                </feed>
                <xsl:for-each select="$categories/category">
                    <feed>
                        <name><xsl:value-of select="name"/></name>
                        <link><xsl:value-of select="local:blog-feed-path(doc('input:instance')/*/username, $blog/blog-id, 'rss20', id)"/></link>
                    </feed>
                </xsl:for-each>
            </feeds>
